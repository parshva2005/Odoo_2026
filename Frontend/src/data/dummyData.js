 ...
 * dummyData.js
 * ─────────────────────────────────────────────────────────────────
 * Single source of truth for ALL mock/dummy data in AssetFlow.
 *
 * HOW IT WORKS (Dummy mode):
 *   Data is seeded into localStorage so changes persist across refreshes.
 *   Each service reads/writes localStorage using the STORAGE_KEYS below.
 *
 * HOW TO SWITCH TO REAL ASP.NET CORE BACKEND:
 *   1. Set VITE_API_BASE_URL=https://your-api.com/api in .env
 *   2. In each service file, delete the "── MOCK ──" block
 *   3. Uncomment the "── REAL ──" block
 *   4. This file is no longer needed once real API is connected.
 * ─────────────────────────────────────────────────────────────────
 */
...
export const STORAGE_KEYS = {
  USERS:              'af_users',
  ASSETS:             'af_assets',
  ALLOCATIONS:        'af_allocations',
  ALLOCATION_HISTORY: 'af_allocation_history',
  BOOKINGS:           'af_bookings',
  MAINTENANCE:        'af_maintenance',
  NOTIFICATIONS:      'af_notifications',
  AUDIT_LOGS:         'af_audit_logs',
  DEPARTMENTS:        'af_departments',
  CATEGORIES:         'af_categories',
  EMPLOYEES:          'af_employees',
  RESOURCES:          'af_resources',
};
...
// ─── Allocation History ───────────────────────────────────────────
export const INITIAL_ALLOCATION_HISTORY = [
...
// ─── Maintenance Tickets (flat array — Kanban derives columns from `status`) ──
export const INITIAL_MAINTENANCE = [
  { id: 1, tag: 'AF-0062', name: 'Projector', issue: 'Projector bulb not turning on', reportedBy: 'Rohan Mehta', date: '2024-07-10', priority: 'High',     status: 'Pending',             tech: null      },
  { id: 2, tag: 'AF-003',  name: 'AC Unit',   issue: 'AC unit noisy compressor',      reportedBy: 'Aditi Rao',   date: '2024-07-09', priority: 'Medium',   status: 'Approved',            tech: null      },
  { id: 3, tag: 'AF-0078', name: 'Forklift',  issue: 'Hydraulic oil leak',            reportedBy: 'Sana Iqbal',  date: '2024-07-08', priority: 'Critical', status: 'Technician Assigned', tech: 'R Varma' },
  { id: 4, tag: 'AF-897',  name: 'Printer',   issue: 'Printer jam parts ordered',     reportedBy: 'Priya Shah',  date: '2024-07-07', priority: 'Low',      status: 'In Progress',         tech: 'R Varma' },
  { id: 5, tag: 'AF-873',  name: 'Chair',     issue: 'Chair repair resolved 7 Jul',   reportedBy: 'Arjun Nair',  date: '2024-07-07', priority: 'Low',      status: 'Resolved',            tech: 'R Varma' },
];
...
// ─── Chart Data ───────────────────────────────────────────────────
export const INITIAL_ASSET_USAGE = [
...
export const INITIAL_MAINTENANCE_CHART = [
...
// ─── Seed localStorage (called once on app startup via main.jsx) ──
export function initializeData() {
  const defaults = [
    [STORAGE_KEYS.DEPARTMENTS,        INITIAL_DEPARTMENTS],
    [STORAGE_KEYS.CATEGORIES,         INITIAL_CATEGORIES],
    [STORAGE_KEYS.EMPLOYEES,          INITIAL_EMPLOYEES],
    [STORAGE_KEYS.ASSETS,             INITIAL_ASSETS],
    [STORAGE_KEYS.ALLOCATIONS,        INITIAL_TRANSFERS],
    [STORAGE_KEYS.ALLOCATION_HISTORY, INITIAL_ALLOCATION_HISTORY],
    ...
    [STORAGE_KEYS.RESOURCES,          INITIAL_RESOURCES],
  ];

  defaults.forEach(([key, data]) => {
