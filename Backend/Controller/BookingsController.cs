...
namespace Backend.Controllers
{
    ...
    public class MaintenanceController : ControllerBase
    {
        ...
        // ==========================================
        // 1. GET MAINTENANCE BOARD (Screen 7)
        // ==========================================
        ...
        public async Task<IActionResult> GetMaintenanceBoard()
        {
            ...
            // Grouping by status makes it very easy for the frontend to render the Kanban columns
            var kanbanBoard = new
            {
        ...
        // ==========================================
        // 3. UPDATE TICKET STATUS (The Side-Effect Engine)
        // ==========================================

        [HttpPut("{requestId}/status")]
        [Authorize(Roles = "Admin,AssetManager")] // Only managers can move cards
        public async Task<IActionResult> UpdateStatus(int requestId, [FromBody] UpdateMaintenanceStatusRequest request)
        {
            ...
            // 2. State Machine Side-Effects on the Asset
            if (newStatus == MaintenanceStatus.Approved || newStatus == MaintenanceStatus.InProgress)
            {
                ...
                maintenanceRecord.Asset.Status = AssetStatus.UnderMaintenance;
            }
            else if (newStatus == MaintenanceStatus.Resolved)
            {
                maintenanceRecord.ResolvedAt = DateTime.UtcNow;
                // Return the asset to circulation
                maintenanceRecord.Asset.Status = AssetStatus.Available;
            }
            ...
            return Ok(new { Message = $"Ticket moved to {newStatus}." });
        }
    }
